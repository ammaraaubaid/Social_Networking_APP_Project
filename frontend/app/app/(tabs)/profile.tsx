...
// ─── Follow Button ────────────────────────────────────────────────────────────
//
// • On mount: calls GET /users/{targetUserId}/follow/status to get initial state
// • On press: calls POST /users/{targetUserId}/follow (server toggles, returns {following})
// • Optimistic UI: flips state immediately, reverts if the request fails
// • Shows a spinner while the initial state is loading or a request is in flight
...
function FollowButton({
  ...
  // null = still fetching initial status
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      async function checkInitialStatus() {
        try {
          const res = await fetch(`${API_URL}/users/${targetUserId}/follow/status`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          if (!cancelled && res.ok) {
            const data = await res.json();
            setIsFollowing(data.following as boolean);
          }
        } catch {
          // Network error — button stays in loading state; not blocking UX
        }
      }
  ...
  async function handleToggle() {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setIsFollowing(data.following as boolean); // confirm with server truth
      } else {
        setIsFollowing(previous); // revert on HTTP error
      }
    } catch {
      setIsFollowing(previous); // revert on network error
    } finally {
      setBusy(false);
    }
  }

  // Still loading initial state → show an outline placeholder with spinner
  if (isFollowing === null) {
    return (
      <View style={[styles.followBtn, styles.followBtnOutline, { borderColor: theme.text + "33" }]}>
        <ActivityIndicator size="small" color={theme.text + "88"} />
      </View>
    );
  }

  const filled  = !isFollowing; // "Follow" = filled, "Following" = outline
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        styles.followBtn,
        filled
          ? [styles.followBtnFilled,  { backgroundColor: theme.text }]
          : [styles.followBtnOutline, { borderColor: theme.text + "44" }],
      ]}
    >
      {busy ? (
        <ActivityIndicator
          size="small"
          color={filled ? theme.background : theme.text}
        />
      ) : (
        <Text style={[styles.followBtnLabel, { color: filled ? theme.background : theme.text }]}>
          {btnText}
        </Text>
      )}
    </TouchableOpacity>
...
// ─── Main Screen ──────────────────────────────────────────────────────────────
//
// Pass `profileUserId` prop when navigating to another user's profile.
// If omitted, the screen shows the logged-in user's own profile (no Follow btn).
